...
/**
 * This is the model class for table "prj_detail".
 *
 * The followings are the available columns in table 'prj_detail':
 ...
 */
class PrjDetail extends CActiveRecord
{
	/**
	 * Returns the static model of the specified AR class.
	 * @param string $className active record class name.
	 * @return PrjDetail the static model class
	 */
	...
	public function rules()
	{
		...
		return array(
			...
			array('Budget, InternalCost, VATTAX', 'length', 'max'=>10),
			array('Status, FStatus, CurrencyNo', 'length', 'max'=>20),
			array('BuyInsAmount, Amount, Balance', 'length', 'max'=>52),
			array('GrossMargin', 'length', 'max'=>54),
			array('PoFile, QuoteFile', 'safe'),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('ProjectNo, SaleManagerNo, SaleManagerName, ProjectManagerNo, ProjectManagerName, ProjectName, Client_ID, ClientName, C_Contact_ID, ContactName, ContactEmail, ContactMobile, C_FContact_ID, FContactName, FContactEmail, FContactMobile, C_PoNo, C_ProjectNo, Budget, Division_ID, DivisionName, Status_ID, Status, FStatus_ID, FStatus, RegDate, DueDate, PoFile, InternalCost, VATTAX, Industry_ID, IndustryName, Password, Currency_ID, CurrencyNo, BuyInsAmount, Amount, Balance, GrossMargin, QuoteFile', 'safe', 'on'=>'search'),
		);
	}
